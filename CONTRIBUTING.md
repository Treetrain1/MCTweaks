...
By contributing to FrozenBlock's MC Tweaks, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across FrozenBlock's MC Tweaks.

If there is more than one acceptable spelling of the same word, first check if one word is already
being used in FrozenBlock's MC Tweaks, FrozenLib, or in Mojang's Mappings, and if not, use the spelling that is most commonly used.
...
It allows to separate the actual name of the variable and the namespace, `mcTweaks`.

Fields marked as `@Unique` must be prefixed with `mcTweaks$` or `MCTWEAKS$` if the field is static and final.
Methods marked with `@Unique` do not need prefixes.


## Licensing & DCO

FrozenBlock's MC Tweaks is licensed under [LGPL 3.0][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE "FrozenBlock's MC Tweaks license file"
